...
class LoginSteps {
	...
	def responseLoginEmpty() {
		WS.verifyResponseStatusCode(statusCode, 400)
	}
	@Given("Post call to url login with empty email")
	def responseLoginEmptyEmail() {
		statusCode = WS.sendRequest(findTestObject('Request/Authentication/Login_empty email'))
	}
	@Given("Post call to url login with empty password")
	def responseLoginEmptyPassword() {
		statusCode = WS.sendRequest(findTestObject('Request/Authentication/Login_empty password'))
	}
	@Given("Post call to url login with unregistered email")
	def responseLoginUnregisteredEmail() {
		WS.verifyElementPropertyValue(statusCode, 'error', "record not found")
	}
}